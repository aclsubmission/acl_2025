    ...
    for data in data_list:
        ...
        if os.path.isfile(ROOT+model.replace('/','')+'_'+data+'.pickle')==False:
            ...
            with open('./liwc_data/'+model+'/'+data+'.csv_liwc.pickle', 'rb') as handle:
                style_vecs = pickle.load(handle)
            style_embedding = []
            for i in range(0,len(style_vecs)):
                style_embedding.append((style_vecs[i][0], style_vecs[i][1], np.random.rand(1, len(style_vecs[i][2]))[0], style_vecs[i][3]))
